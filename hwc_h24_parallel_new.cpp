...
int main()
{
	...
	dataBuf = JpegFile::JpegFileToRGB("/Users/kevinmueller/Documents/hpc_hw3/hpc_hw3/testcolor.jpg", &width, &height);
    ...
    for(int iteration =0;iteration<10;iteration++){
        ...
        #pragma omp parallel for private(row) reduction (+:sum)
        for(row=0;row<height;row++){
            for(col=0;col<width;col++){
                if(group_indices[row*width+col] ==0){
    ...
    for(int row=0;row<height;row++){
        for(int col=0;col<width;col++){
            ...
            out[row * width * 3 + col * 3] = (BYTE)temp_gen.r;
            out[row * width * 3 + col * 3+1] = (BYTE)temp_gen.g;
            out[row * width * 3 + col * 3+2] = (BYTE)temp_gen.b;
	...
	JpegFile::RGBToJpegFile("/Users/kevinmueller/Documents/hpc_hw3/hpc_hw3/testmono.jpg", out, width, height, true, 75);
